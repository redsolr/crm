...
describe("buildWeekBuckets", () => {
  ...
  it("buckets by call_date when present, falling back to created_at", () => {
    // Both notes were typed in during the current week; n1 records a
    // call that HAPPENED the week before.
    const notes = [
      callNote("n1", "2026-07-14T10:00:00"),
      callNote("n2", "2026-07-14T11:00:00"),
    ];
    const buckets = buildWeekBuckets(
      notes,
      NOW,
      { n1: "2026-07-10" }, // previous week (local midnight)
      8,
    );
    expect(buckets[7]!.count).toBe(1); // n2 via created_at
    expect(buckets[6]!.count).toBe(1); // n1 via call_date
  });
});
