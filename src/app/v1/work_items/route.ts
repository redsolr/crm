...
/**
 * `GET /v1/work_items` (cursor list) + `POST /v1/work_items` (create) —
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  return NextResponse.json({
    ...
    next_page_url: hasMore
      ? `/v1/work_items?page_token=${encodePageToken(offset + pageSize)}`
      : null,
    previous_page_url:
      offset > 0
        ? `/v1/work_items?page_token=${encodePageToken(Math.max(0, offset - pageSize))}`
        : null,
  });
}
