...
export function useMenuNavigation<T>({
  ...
  // Reset selection when the search query changes — adjusting state
  // during render rather than via a useEffect cascade. Tracks both
  // `query` and `autoSelectFirstItem` so a switch in either resets
  // the selection (matches the prior effect's dep list).
  const [lastQueryState, setLastQueryState] = useState({
    query,
    autoSelectFirstItem,
  });
  if (
    lastQueryState.query !== query ||
    lastQueryState.autoSelectFirstItem !== autoSelectFirstItem
  ) {
    if (query) {
      setSelectedIndex(autoSelectFirstItem ? 0 : -1);
    }
    setLastQueryState({ query, autoSelectFirstItem });
  }
