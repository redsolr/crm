...
export function useElementRect({
  ...
  // Reset rect to the zero baseline whenever tracking is disabled.
  // Adjusting state during render so the disabled-flip doesn't go
  // through a setState-in-effect cascade.
  const [lastEnabled, setLastEnabled] = useState(enabled);
  if (enabled !== lastEnabled) {
    if (!enabled) setRect(initialRect);
